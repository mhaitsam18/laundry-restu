...
class Produk extends CI_Controller {
	...
	{
		$data['title'] = "Data Keluar Masuk Produk";
		$this->db->select('*');
		$this->db->join('produk', 'produk.id_produk=inout_produk.id_produk');
		$data['inout_produk'] = $this->db->get('inout_produk')->result_array();
		$data['produk'] = $this->db->get('produk')->result_array();
		$data['user'] = $this->db->get_where('user', ['email' => $this->session->userdata('email')])->row_array();
		...
		if ($this->form_validation->run() == false) {
			...
			$this->session->set_flashdata('message', '<div class="alert alert-success" role="alert">Data Barang diperbarui!</div>');
