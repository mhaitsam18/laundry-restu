...
class Transaksi extends CI_Controller {
	...
	public function tambahKeranjang($id_produk, $rowid = '')
	{
	...
	public function updateStatusPesanan($id_checkout, $status = '')
	{
		...
		$checkout = $this->db->get_where('checkout', ['id_checkout' => $id_checkout])->row();
	...
	public function updateStatusPembayaran($id_bukti_transfer, $status = '')
	{
		$user = $this->db->get_where('user', ['email' => $this->session->userdata('email')])->row();
		$status = urldecode($status);
		$this->db->where('id_bukti_transfer', $id_bukti_transfer);
		$this->db->update('bukti_transfer', ['status' => 'Pembayaran '.$status]);
		if ($status == 'valid') {
			$bukti_transfer = $this->db->get_where('bukti_transfer', ['id_bukti_transfer' => $id_bukti_transfer])->row_array();
			$this->db->where('id_checkout', $bukti_transfer['id_checkout']);
			$this->db->update('checkout', ['status' => 'Sudah dibayar']);
		}

		$bukti_transfer = $this->db->get_where('bukti_transfer', ['id_bukti_transfer' => $id_bukti_transfer])->row();
		$checkout = $this->db->get_where('checkout', ['id_checkout' => $bukti_transfer->id_checkout])->row();

		$this->db->insert('notifikasi', [
			'id_user' => $checkout->id_user,
			'id_kategori_notifikasi' => 3,
			'sub_id' => $id_bukti_transfer,
			'waktu_notifikasi' => date('Y-m-d H:i:s'),
			'subjek' => 'Pembayaran',
	...
	public function updateBeban()
	{
		...
		if ($this->form_validation->run() == false) {
			...
			$this->db->where('id_beban', $this->input->post('id_beban'));
	...
	{
		$this->db->delete('beban', ['id_beban' => $id_beban]);
		$this->session->set_flashdata('message', '<div class="alert alert-success" role="alert">
			Load Deleted!
			</div>');
		redirect('Transaksi/beban');
